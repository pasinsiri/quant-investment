...
class Backtest():
    ...
    def __init__(
            ...
            alpha_factors: list,
            risk_factors: list,
            n_forward_return: int,
            risk_aversion_coefficient: float,
            date_index: int = 1) -> None:
        ...
        self.join_df = self._map_forward_return(
            n_forward_return=n_forward_return)
        self.df_dict_by_date = self._split_to_date(
            self.join_df, date_index=self.date_index)
        self.factor_return_df = {date: self.estimate_factor_returns(
            self.df_dict_by_date[date]).params for date in self.df_dict_by_date}
    ...
    def _map_forward_return(self, n_forward_return: int):
        ...
        dates_df = self.return_df[['return_date']].sort_values(
            by='return_date').drop_duplicates().reset_index(drop=True)
        dates_df['date'] = dates_df['return_date'].shift(n_forward_return)
        self.return_df = self.return_df.merge(
            dates_df, on='return_date', how='left')
    ...
    def estimate_factor_returns(
            self,
            df,
            return_col: str = 'DlyReturn',
            lower_bound: float = -0.25,
            upper_bound: float = 0.25,
            factor_keyword: str = 'USFASTD_'):
        ...
        df[return_col] = self._winsorize(
            df[return_col], lower_bound, upper_bound)
    ...
    def diagonal_factor_cov(self, date, B):
        ...
        for i in range(k):
            cov_value = cov[(cov['Factor1'] == factor_names[i]) & (
                cov['Factor2'] == factor_names[i])]['VarCovar']
            # convert to decimal since the raw data comes in the pct squared
            # format
            factor_cov[i, i] = 1e-4 * cov_value
        return factor_cov
    ...
    def get_lambda(self, universe, composite_volume_column: str = 'ADTCA_30'):
        ...
        adv.loc[np.isnan(adv[composite_volume_column]),
                composite_volume_column] = 1.0e4
        adv.loc[adv[composite_volume_column] ==
                0, composite_volume_column] = 1.0e4
        return 0.1 / adv
    ...
    def get_obj_func(self, h0, Q, specVar, alpha_vec, Lambda):
        def obj_func(h):
            ...
            Qh = Q @ h
            factor_risk = 0.5 * \
                self.risk_aversion_coefficient * np.sum(Qh ** 2)
            idiosyncratic_risk = 0.5 * \
                self.risk_aversion_coefficient * ((h ** 2) @ specVar)
    ...
    def get_risk_exposures(self, B, h_star):
        ...
        return pd.Series(
            B.transpose() @ h_star,
            index=B.design_info.column_names)
    ...
    def form_optimal_portfolio(self, df, previous):
        ...
        opt_portfolio = pd.DataFrame(
            data={
                "Barrid": universe['Barrid'],
                "h.opt": h_star})

        risk_exposures = self.get_risk_exposures(B, h_star)
        portfolio_alpha_exposure = self.get_portfolio_alpha_exposure(
            B_alpha, h_star)
    ...
    def build_tradelist(self, prev_holdings, opt_result):
        tmp = prev_holdings.merge(
            opt_result['opt.portfolio'],
            how='outer',
            on='Barrid')
...
class SimpleBacktest():
    def __init__(
            ...
            trading_cost_in: float = 0.0,
            trading_cost_out: float = 0.0) -> None:
        self.factor_df = factor_df
