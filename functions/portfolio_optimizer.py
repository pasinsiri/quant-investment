...
class RiskModelPCA():
    def __init__(self, returns, annualization_factor, num_factor_exposures, svd_solver:str = 'full'): 
        self.returns = returns 
        self.ticker_list = returns.columns
        self.return_dates = returns.index
        self.pca_factor_names = [f'PC_{i}' for i in range(num_factor_exposures)]
        self.annualization_factor = annualization_factor
        self.pca = PCA(n_components = num_factor_exposures, svd_solver = svd_solver).fit(returns)
    ...
    def get_factor_beta(self):
        return pd.DataFrame(self.pca.components_.T, index = self.ticker_list, columns = self.pca_factor_names)

    def get_factor_return(self):
        return pd.DataFrame(self.pca.transform(self.returns), index = self.return_dates, columns = self.pca_factor_names)

    def get_idiosyncratic_var_matrix(self):
        common_returns_ = pd.DataFrame(np.dot(self.factor_returns_, self.factor_betas_.T), index = self.return_dates, columns = self.ticker_list)
        residuals_ = (self.returns - common_returns_)
        return pd.DataFrame(np.diag(np.var(residuals_)) * self.annualization_factor, index = self.ticker_list, columns = self.ticker_list)

    def get_factor_cov_matrix(self):
        return np.diag(self.factor_returns_.var(axis=0, ddof=1) * self.annualization_factor)
...
class OptimalHoldings(AbstractOptimalHoldings):        
    ...
    def __init__(self, lambda_reg:float = 0.0, risk_cap:float = 0.05, factor_max:float = 10.0, factor_min:float = -10.0, weights_max:float = 0.55, weights_min:float = -0.55):
