...
class OHLCVCleaner():
    ...
    def _adjust_ohlc_single_ticker(self, ticker_df, replace: bool):
        ...
        ticker_df['stock splits'] = ticker_df['stock splits'].apply(
        ...
        for col in ['open', 'high', 'low', 'close']:
            new_col = self._gen_new_col_name(
                col=col, prefix='adj_', replace=replace)
            ticker_df[new_col] = ticker_df.apply(
                lambda row: (
                    row['close'] *
                    row['accum_stock splits']) +
                row['accum_dividends'],
                axis=1)
    ...
    def adjust_ohlc(self, ohlc, replace: bool = False):
        ...
        assert len(
            unexisted) == 0, f'{", ".join(unexisted)} not found in the OHLC dataframe'
        ...
        for ticker in tickers:
            ...
            adjusted_ticker_df = self._adjust_ohlc_single_ticker(
                ticker_df=ticker_df, replace=replace)
