...
class MonteCarloSimulator():
    ...
    def simulate_outstanding(
        ...
        for i in range(self.n_iteration):
            ...
            for y in range(self.n_period + 1):
                ...
                balances.append([(i + 1), y, starting_outstanding,
                                accumulated_contribution, ending])

                if to_pandas:
                    column_names = [
                        'iteration', 'month', 'start', 'contribution', 'end']
                    balances = pd.DataFrame(balances, columns=['_'.join(
                        [c, str(i)]) if c != 'month' else c for c in column_names]).set_index('month')
            ...
            if to_pandas:
                ...
                if all_ending_balances is None:
                    ...
                    all_ending_balances = all_ending_balances.merge(
                        balances, left_index=True, right_index=True)
    ...
    def get_stat_values(
        ...
        start_pcts = np.array([np.apply_along_axis(
            lambda x: np.percentile(x, q), 0, start_values) for q in percentiles])
        contrib_pcts = np.array([np.apply_along_axis(
            lambda x: np.percentile(x, q), 0, contrib_values) for q in percentiles])
        pcts = np.concatenate(
            (periods, start_pcts, contrib_pcts), axis=0).transpose()
        if to_pandas:
            col_names = ['month']
            col_names.extend([f'{keyword}_percentile_{q}' for keyword in [
                             'initial', 'contribution'] for q in percentiles])
    ...
    def gen_wealth_path(
        ...
        percentiles: list = [
            ...
            75,
            95]):
        simulation = self.simulate_outstanding(
            initial_amt=initial_amt,
            contribution=contribution,
            mean=mean,
            stdev=stdev)
        pcts = self.get_stat_values(
            simulation=simulation,
            percentiles=percentiles,
            to_pandas=True)
        return pcts

    def gen_multiple_wealth_path(
        self,
        path: str,
        initial_amt: float,
        contribution: float,
        ...
        for row in [r[1] for r in df.iterrows()]:
            wealth_path = self.gen_wealth_path(
                initial_amt=initial_amt,
                contribution=contribution,
                mean=row['expected_return'],
                stdev=row['stdev'],
                percentiles=percentiles)
            ...
            if all_wealth_path is None:
                ...
                all_wealth_path = all_wealth_path.append(
                    wealth_path, ignore_index=True)
...
class MonteCarloSimulator_old():
    ...
    def simulate_outstanding(
        ...
        for i in range(n_iteration):
            ...
            column_names = [
                '_'.join([c, str(i + 1)]) if c != 'month' else c for c in column_format]
            simulation_df = pd.DataFrame(
                sim_values, columns=column_names).set_index('month')

            if all_df is None:
                all_df = simulation_df.astype(float)
            else:
                all_df = all_df.merge(
                    simulation_df, left_index=True, right_index=True)

            if verbose:
                if (i + 1) % 100 == 0:
                    print(
                        f'Iteration {i+1} is completed, {n_iteration - (i+1)} iterations left')
        return all_df
    ...
    def get_stat_values(
            ...
            percentiles: list = [
                ...
                75,
                95],
            join_to_df: bool = False) -> pd.DataFrame:
        if sim_df is None:
        ...
        for q in percentiles:
            stat_df.loc[:, f'pct_{q}'] = ending_balance_df[init_cols].apply(
                lambda x: np.percentile(x, q=q), axis=1)
    ...
    def plot_wealth_path(
        ...
        if len(pct_cols) != len(colors):
            raise ValueError(
                'number of colors is not equal to number of lines in the plot')
        ...
        ax.set_title(
            f'Monte Carlo Simulation for {self.n_iteration} iterations')
        plt.legend()
