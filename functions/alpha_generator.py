...
class NoOverlapClassifier(NoOverlapClassifierAbstract):
    ...
    def _non_overlapping_estimators(self, x, y, classifiers, n_skip_samples):
        ...
        samples = [self._non_overlapping_samples(x, y, n_skip_samples, start_i) for start_i in range(n_classifiers)]
        fit_classifiers = [classifiers[i].fit(sample[0], sample[1]) for i, sample in enumerate(samples)]
