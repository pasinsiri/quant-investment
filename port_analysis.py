...
my_stock['portfolio_weight'] = my_stock.apply(
    lambda x: x['percent'] * x['percent_asset'] / 100.0, axis=1)

top_stocks = my_stock[['full_name', 'portfolio_weight']].groupby(
    'full_name').sum().sort_values(by='portfolio_weight', ascending=False)

# * my sector weights
my_sector = my_port.merge(sectors, on='fund', how='left')
my_sector['percent_asset'].fillna(100, inplace=True)
my_sector['sector_weight'] = my_sector.apply(
    lambda x: x['percent'] * x['percent_asset'] / 100.0, axis=1)

top_sector = my_sector[['sector', 'sector_weight']].groupby(
    'sector').sum().sort_values(by='sector_weight', ascending=False)

# * my region weights
my_region = my_port.merge(regions, on='fund')
my_region['percent_asset'].fillna(100, inplace=True)
my_region['region_weight'] = my_region.apply(
    lambda x: x['percent'] * x['percent_asset'] / 100.0, axis=1)
top_region = my_region[['region', 'region_weight']].groupby(
    'region').sum().sort_values(by='region_weight', ascending=False)
